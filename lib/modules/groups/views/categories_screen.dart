// lib/modules/groups/views/categories_screen.dart
...
import '../providers/categories_provider.dart'; // 🚀 استيراد المزود
...
class _CategoriesScreenState extends ConsumerState<CategoriesScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _searchController.addListener(() => setState(() {})); // التحديث لتفعيل فلترة البحث
                    ...
                    Container(
                      ...
                      child: Row(children: [
                        Container(
                          ...
                          child: Icon(isEditing ? Icons.edit_rounded : Icons.create_new_folder_rounded, color: AppColors.secondary, size: 20),
                        ),
                        const SizedBox(width: 12),
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(isEditing ? 'تعديل فئة' : 'إضافة فئة جديدة', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                            Text(isEditing ? 'تحديث بيانات الفئة' : 'أدخل معلومات الفئة الجديدة', style: TextStyle(fontSize: 11, color: AppColors.primary.withOpacity(0.8))),
                          ],
                        ),
                      ]),
                    ),
                    const SizedBox(height: 22),
                    _fieldLabel('اسم الفئة', colors),
                    const SizedBox(height: 6),
                    _buildInputField(controller: nameController, hint: 'مثال: هواتف', icon: Icons.label_outline_rounded, colors: colors),
                    const SizedBox(height: 16),
                    _fieldLabel('الوصف', colors),
                    const SizedBox(height: 6),
                    _buildInputField(controller: descController, hint: 'وصف الفئة (اختياري)', icon: Icons.description_outlined, maxLines: 3, colors: colors),
                      ...
                      Expanded(
                        ...
                        child: ElevatedButton.icon(
                          onPressed: () async {
                            ...
                            // 🚀 توجيه الأوامر إلى الـ Provider
                            final notifier = ref.read(categoriesProvider(widget.groupId).notifier);
                            final data = {
                              'name': nameController.text.trim(),
                              'description': descController.text.trim(),
                              if (!isEditing) 'group_id': widget.groupId,
                            };

                            if (isEditing) {
                              await notifier.updateCategory(category!['id'], data);
                            } else {
                              await notifier.addCategory(data);
                            }
  ...
  void _confirmDelete(Map<String, dynamic> category) async {
    ...
    // 🚀 الحماية المعمارية: منع حذف فئة تحتوي على أصناف
    final hasLinkedSubcategories = await notifier.hasSubcategories(category['id']);
    if (hasLinkedSubcategories && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('لا يمكن الحذف: يوجد أصناف (Subcategories) مرتبطة بهذه الفئة'),
        backgroundColor: AppColors.warning.withOpacity(0.95),
      ));
      return;
    }
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text('حذف الفئة', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: _colors.textMain)),
  ...
  Widget build(BuildContext context) {
    final colors = _colors;
    // 🚀 الاستماع للحالة بشكل مباشر
    final asyncCategories = ref.watch(categoriesProvider(widget.groupId));
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('فئات ${widget.groupName}', style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold, fontSize: 16)),
        ]),
        actions: [
          IconButton(
        ...
        bottom: PreferredSize(
          ...
          child: Container(height: 2, decoration: BoxDecoration(gradient: LinearGradient(colors: [AppColors.navy, AppColors.secondary.withOpacity(0.6), AppColors.navy]))),
        ),
      ),
      body: _buildBody(colors, asyncCategories),
  ...
  Widget _buildBody(AppThemeColors colors, AsyncValue<List<Map<String, dynamic>>> asyncCategories) {
    return Column(children: [
      if (_showSearch)
        Container(
          padding: const EdgeInsets.fromLTRB(16, 12, 16, 8),
          color: colors.scaffoldBg == AppColors.navy ? AppColors.navyMedium.withOpacity(0.5) : AppColors.secondary.withOpacity(0.05),
          child: TextField(
            controller: _searchController, autofocus: true,
            style: TextStyle(color: colors.textMain, fontSize: 14),
            decoration: InputDecoration(
              hintText: 'بحث باسم الفئة أو الوصف...', hintStyle: TextStyle(color: colors.textHint, fontSize: 13),
      ...
      Expanded(
        child: asyncCategories.when(
          loading: () => const Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(AppColors.secondary))),
          ...
          data: (categories) {
            ...
            final filteredList = query.isEmpty
                ? categories
                : categories.where((c) => (c['name'] ?? '').toLowerCase().contains(query) || (c['description'] ?? '').toLowerCase().contains(query)).toList();
            ...
            if (categories.isEmpty) {
              return Center(
                child: Column(
                  ...
                  children: [
                    Icon(Icons.folder_open_rounded, size: 60, color: colors.textHint),
                    const SizedBox(height: 16),
                    Text('لا توجد فئات', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: colors.textMain)),
                    const SizedBox(height: 4),
                    Text('اضغط على زر + لإضافة فئة', style: TextStyle(fontSize: 12, color: colors.textHint)),
                  ],
                ),
              );
            }
            ...
            return RefreshIndicator(
              onRefresh: () async => ref.refresh(categoriesProvider(widget.groupId)),
              ...
              child: FadeTransition(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) => _buildCategoryCard(filteredList[index], colors),
                ),
              ),
            );
          },
  ...
  Widget _buildCategoryCard(Map<String, dynamic> category, AppThemeColors colors) {
    return Container(
      margin: const EdgeInsets.only(bottom: 10),
      child: InkWell(
        onTap: () => Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => SubcategoriesScreen(categoryId: category['id'], categoryName: category['name'])),
        ),
        borderRadius: BorderRadius.circular(16),
        child: Container(
          padding: const EdgeInsets.all(14),
          ...
          child: Row(children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(category['name'] ?? '', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: colors.textMain)),
                  if (category['description'] != null && category['description'].toString().isNotEmpty) ...[
                    const SizedBox(height: 3),
                    Text(category['description'], style: TextStyle(fontSize: 12, color: colors.textSub), maxLines: 1, overflow: TextOverflow.ellipsis),
                  ],
                ],
              ),
            ),
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: Icon(icon, color: AppColors.secondary, size: 20), border: InputBorder.none,
